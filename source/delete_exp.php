...
if($_SERVER["REQUEST_METHOD"] == "POST"){
	...
	// if no box is checked, the delete variable will not exist, redirect to the same page
		header("Location:delete_exp.php");
	}else{
	// carry out the soft delete - use loop
		$total = 0; // for the total number of expences deleted
		...
		for($affect=0; $affect<count($_POST["delete"]); $affect++){
			...
			$sql = "
					UPDATE `expences`
					SET `exp_deleted`='1'
					WHERE`exp_id`='" . $_POST["delete"][$affect] . "';
					";
					// echo $sql;
					// echo "<br>";
...
<head>
	...
	<link href="css/delete_exp.css" type="text/css" rel="stylesheet"><!-- css - delete expences -->
...
?>
	...
	<?php
		...
		// retrieve data
		$data = mysqli_fetch_all($result, MYSQLI_BOTH) or die("It seems like you don't have any expences yet<br><a class='empty' href='home.php'>Go to home page</a>");
			// echo "<pre>";
			// var_dump($data);
			// echo "</pre>";
			// echo count($data);
		...
		<form method="post">
			...
			?>		
				<tr class="hover">
					<td class="del-col">
						<!-- checkbox -->
						<input type="checkbox" name="delete[]" value="<?php echo $data[$row]["exp_id"]; ?>">
					</td>
					<td class="num"><?php echo $row+1; ?></td><!-- numbering -->
					<td class="name"><?php echo $data[$row]["exp_name"]; ?></td><!-- payment name -->
